...
export default {
  ...
  methods: {
    renderModel (poster, sourceUrl) {
      this.modelViewer = `<model-viewer style="width: ${this.viewWidth};height: ${this.viewHeight};"  autoplay="${isAutoPlay}" camera-controls="true" animation-name="Run" interpolation-decay="200" max-camera-orbit="auto 75deg auto" min-camera-orbit="auto 75deg auto"  disable-zoom="true" id="characterMviewer" class="characterMviewer" poster="${poster}" src="${sourceUrl}" ar-status="not-presenting"></model-viewer>`;
    },
  }
}
